...
namespace FsDataAccess.Configurations
{
    public class FinancialStatementConfiguration : IEntityTypeConfiguration<FinancialStatement>
    {
        ...
        public FinancialStatementConfiguration(string schema, bool useHistoryTable = false)
        {
            _schema = schema;
            _useHistoryTable = useHistoryTable;
        }

        public void Configure(EntityTypeBuilder<FinancialStatement> entity)
        {
            entity.HasKey(e => e.Id).HasName("PK_dbo_FinancialStatements");
