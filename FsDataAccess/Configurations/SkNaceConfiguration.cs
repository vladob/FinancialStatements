...
{
    public class SkNaceConfiguration : IEntityTypeConfiguration<SkNace>
    {
        private readonly string _schema;
        private readonly bool _useHistoryTable;

        public SkNaceConfiguration(string schema, bool useHistoryTable = false)
        {
            _schema = schema;
            _useHistoryTable = useHistoryTable;
        }

        public void Configure(EntityTypeBuilder<SkNace> entity)
        {
            entity.HasKey(e => e.Id).HasName($"PK_{_schema}_SkNace");

            entity.ToTable("SkNace", _schema);

            if (_useHistoryTable)
            {
                entity.ToTable(tb => tb.IsTemporal(ttb =>
                {
                    ttb.UseHistoryTable("SkNaceHistory", "versioning");
